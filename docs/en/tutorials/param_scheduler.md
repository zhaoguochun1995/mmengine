...
During neural network training, optimization hyperparameters, such as learning rate, are often adjusted along with the training process.
One of the simplest and most common learning rate adjustment strategy is multi-step learning rate decay,
which reducing the learning rate to a fraction at regular intervals.
PyTorch provides LRScheduler to abstract various learning rate adjustment methods, but the function is still limited.

In MMEngine, we have extended it and implemented a more general parameter scheduler `mmengine.optim.scheduler`.
It can adjust optimization hyperparameters such as learning rate and momentum. And supports the combination of multiple schedulers to create more complex scheduling strategies.

## Usage

We first introduce how to use PyTorch's `torch.optim.lr_scheduler` to adjust learning rate. Here is an example which refers from [PyTorch official documentation](https://pytorch.org/docs/stable/optim.html):
...
`mmengine.optim.scheduler` supports most of PyTorch's learning rate schedulers such as `ExponentialLR`，`LinearLR`，`StepLR`，`MultiStepLR`, etc. All of the supported schedulers please refer to [parameter scheduler API documentation](mmengine.optim.scheduler).
...
If using the runner with the registry and config file, we can specify the optimizer by setting the `param_scheduler` field in the config. The runner will automatically build a parameter scheduler based on this field:
...
In addition to learning rate and momentum, users can also schedule other parameter names in `optimizer.param_groups`. The schedulable parameters depend on the optimizer used. For example, when using the SGD optimizer with `weight_decay`, the `weight_decay` can be adjusted as follows:
