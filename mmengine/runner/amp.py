...
def autocast(device_type: Optional[str] = None,
    ...
    assert 'parrots' == TORCH_VERSION or ('parrots' != TORCH_VERSION and digit_version(TORCH_VERSION) >= digit_version('1.5.0')), (
        'The minimum pytorch version requirements of mmengine is 1.5.0, but '
        f'got {TORCH_VERSION}')

    if ('parrots' == TORCH_VERSION or ( 'parrots' != TORCH_VERSION and digit_version('1.5.0') <= digit_version(TORCH_VERSION) <
